...
exports.updateStore = async (req, res) => {
  ...
  const store = await Store.findOneAndUpdate({ _id: req.params.id }, req.body, {
    new: true,  // this returns the new store instead of the old one
    runValidators: true,
  }).exec();
...
exports.getStoreBySlug = async (req, res, next) => {
  ...
  if (!store) return next(); // if the query isn't found, next to 404 ;)
...
exports.heartsPage = async (req, res) => {
  ...
  res.render('stores', {stores,  title: 'Hearted by you:'})
};
